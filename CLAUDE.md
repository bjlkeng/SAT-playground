...
6. **Iterate** (at least 10 attempts unless the user asked for a narrower experiment): Make one change at a time, benchmark it against the current accepted baseline, and keep it only if it improves the target metric by more than `3%`. For profiling-suite optimization, the target metric is aggregate PAR-2 only: instance-level solved/timeout churn is diagnostic context, not a rejection criterion, when aggregate PAR-2 improves. Recalculate the accepted baseline and keep threshold after every kept change. Revert changes that improve PAR-2 by `3%` or less, worsen aggregate PAR-2, fail correctness checks, or only shift time into a slow new implementation.
