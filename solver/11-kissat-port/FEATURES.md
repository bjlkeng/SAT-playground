...
| `SAT_SEARCH_MODE` | `search_mode_policy` | Experimental | none | `log/bench-11-kissat-port-2026-05-25-20-01-30/results.csv` | Focused/stable mode overlay. Env-facing `focused-stable` requests execute the actual focused/stable path with LBD enabled and focused-only VMTF by default. |
...
| `SAT_LUCKY` | `lucky` | SmokeSafe | default, fast | `log/bench-11-kissat-port-2026-05-25-22-30-00/results.csv` | Pre-search lucky assignment pass. Default and fast profiles try all-true/all-false, forward/backward false/true temporary propagation probes, then a bounded small-formula local repair fallback; `SAT_LUCKY=off` restores the previous direct-CDCL path. |
