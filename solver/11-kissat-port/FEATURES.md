...
| `SAT_VMTF` | `vmtf` | SmokeSafe | none | `log/phase1/egy-vmtf-single-profile` | Optional Variable-Move-To-Front branch queue. Use `SAT_VMTF=single` for single-mode search; legacy `SAT_VMTF=on` maps to `focused-only` and requires `SAT_USE_LBD=on SAT_SEARCH_MODE=focused-stable`. |
...
| `SAT_SEARCH_MODE` | `search_mode_policy` | SmokeSafe | none | `log/phase1/5b2.2.52-s11-default-clean` | `focused-stable` overlays `SAT_PHASE`: focused mode maps `legacy`/`saved` to `saved` and target/best-target inputs to `target-then-saved`; stable mode always uses `best-then-target-then-saved`. It remains opt-in until it beats the solver 10 baseline. |
