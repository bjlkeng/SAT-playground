...
| `SAT_SEARCH_MODE` | `search_mode_policy` | SmokeSafe | none | `log/1.9/summary.md` | `focused-stable` overlays `SAT_PHASE`: focused mode maps `legacy`/`saved` to `saved` and target/best-target inputs to `target-then-saved`; stable mode always uses `best-then-target-then-saved`. |
