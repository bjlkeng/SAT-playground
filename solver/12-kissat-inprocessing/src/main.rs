...
/// Bead SAT-playground-5b2.3.38: random-simulation candidate discovery.
/// Number of 64-bit simulation words per variable (=> 64*W random parallel patterns);
/// more bits => fewer spurious signature collisions => fewer wasted kitten proofs.
const SIM_WORDS: usize = 4;
/// Max passes to propagate simulation values through the gate DAG (converges in <= DAG
/// depth for acyclic gate graphs; capped for the rare cyclic extraction artifact).
const SIM_MAX_PASSES: usize = 128;
/// Max candidate pairs a single simulation round hands to the (per-pair-environment)
/// prover — bounds the sweeping cost when a signature bucket is very large.
const SIM_MAX_PAIRS: usize = 256;
...
struct Solver {
    ...
    sweep: bool,
    /// Bead SAT-playground-5b2.3.38: env SAT_SWEEP_SIM. Random-simulation candidate-
    /// equivalent-pair discovery for sweeping. Structural-hash discovery (congruence) only
    /// finds structurally-identical gate outputs and regresses medium; the miter-gap
    /// equivalences are SEMANTIC (structurally-different circuits computing the same
    /// function). This pass simulates K random primary-input assignments through the
    /// extracted gate structure, groups variables by their K-bit output signature, and for
    /// each equal/complementary-signature candidate pair builds a two-seed environment
    /// spanning BOTH fan-in cones and hands it to the sound kitten+RUP+ELS pipeline. Every
    /// applied fact is still proven by an UNSAT kitten call; simulation only selects which
    /// pairs to try. Requires SAT_INPROCESS=on; default off. Correctness-critical (DRAT).
    sweep_sim: bool,
    ...
    /// local preprocessing work ticks allowed for BSR + BVE; zero means unlimited
...
impl Solver {
    ...
    fn new_with_config(num_vars: usize, clauses: Vec<Vec<i32>>, config: &SolverConfig) -> Self {
        ...
        let mut solver = Solver {
            ...
            sweep_sim: matches!(
                std::env::var("SAT_SWEEP_SIM").as_deref(),
                Ok("on") | Ok("1") | Ok("true")
            ),
    ...
    #[inline(always)]
    fn record_propagation_accounting<const NORMAL_SEARCH: bool>(&mut self) {
        if NORMAL_SEARCH {
            self.stats.propagations += 1;
        } else if self.accounting_mode.is_temporary() {
    ...
    #[inline]
    fn record_search_ticks<const MODE_TICKS: bool, const NORMAL_SEARCH: bool>(
        &mut self,
        ticks: u64,
    ) {
        if MODE_TICKS && NORMAL_SEARCH {
    ...
    fn enqueue(&mut self, lit: i32, reason: ReasonRef) -> bool {
        if self.accounting_mode.is_temporary() {
            self.enqueue_impl::<false>(lit, reason)
        } else {
            self.enqueue_impl::<true>(lit, reason)
        }
    }
    ...
    >(
        ...
        let normal_search_accounting = NORMAL_SEARCH;
    ...
    fn inprocess_round(&mut self, proof_log: &mut ProofLog, config: &SolverConfig) -> bool {
        ...
        if ok && self.sweep_sim {
            ok = self.sweep_simulation_round(proof_log);
        }
    ...
    /// Random-simulation candidate-equivalent-pair discovery (bead SAT-playground-5b2.3.38).
    ///
    /// Returns candidate pairs `(a, b_signed)` with `|a| < |b|`: `b_signed = +b` means the
    /// pair is a candidate `a <-> b`, `b_signed = -b` a candidate `a <-> ¬b`. Candidates are
    /// variables whose K-bit simulation signatures agree (equal or complementary) across all
    /// random patterns — a *necessary* condition for equivalence that structural hashing
    /// misses when the two circuits are not syntactically identical. Every returned pair is
    /// only a *candidate*: the caller proves or rejects it with an exact kitten SAT call, so
    /// simulation soundness is irrelevant — it only steers which pairs are worth an
    /// environment.
    fn sweep_simulation_candidate_pairs(&self) -> Vec<(i32, i32)> {
        use std::collections::HashMap;
        let num_vars = self.assignment.len().saturating_sub(1);
        if num_vars == 0 {
            return Vec::new();
        }
        let gates = self.extract_gates_for_congruence(true);
        if gates.is_empty() {
            return Vec::new();
        }

        // First gate seen wins per output variable; evaluate in extraction order.
        let mut has_gate = vec![false; num_vars + 1];
        let mut ordered: Vec<&congruence::Gate> = Vec::with_capacity(gates.len());
        for g in &gates {
            let ov = g.out.unsigned_abs() as usize;
            if ov == 0 || ov > num_vars || has_gate[ov] {
                continue;
            }
            has_gate[ov] = true;
            ordered.push(g);
        ...
        // Deterministic PRNG (splitmix64) — no Date/random available in this build. Seeding
        // per (var, word) makes the signatures reproducible run-to-run.
        ...
        // Random initial words for every variable; gate outputs get overwritten below, so
        // only primary inputs (and free vars) retain their random patterns.
        let mut values: Vec<[u64; SIM_WORDS]> = vec![[0u64; SIM_WORDS]; num_vars + 1];
        for v in 1..=num_vars {
            for w in 0..SIM_WORDS {
                values[v][w] = mix((v as u64)
                    .wrapping_mul(SIM_WORDS as u64)
                    .wrapping_add(w as u64)
                    .wrapping_add(0x1234_5678));
            }
        }

        let lit_word = |values: &Vec<[u64; SIM_WORDS]>, lit: i32, w: usize| -> u64 {
            let raw = values[lit.unsigned_abs() as usize][w];
            if lit < 0 {
                !raw
            } else {
                raw
        ...
        // Propagate gate functions to a fixpoint (converges in <= DAG depth when acyclic).
        for _ in 0..SIM_MAX_PASSES {
            let mut changed = false;
            for g in &ordered {
                let ov = g.out.unsigned_abs() as usize;
                let mut newv = [0u64; SIM_WORDS];
                match g.kind {
                    congruence::GateKind::And => {
                        for w in 0..SIM_WORDS {
                            let mut acc = u64::MAX;
                            for &inp in &g.inputs {
                                acc &= lit_word(&values, inp, w);
                            }
                            newv[w] = acc;
                        }
                    }
                    ...
                    congruence::GateKind::Ite => {
                        ...
                        for w in 0..SIM_WORDS {
                            let c = lit_word(&values, g.inputs[0], w);
                            let t = lit_word(&values, g.inputs[1], w);
                            let e = lit_word(&values, g.inputs[2], w);
                            newv[w] = (c & t) | (!c & e);
                        }
                    }
                }
                // out literal carries the gate's output polarity.
                if g.out < 0 {
                    for w in 0..SIM_WORDS {
                        newv[w] = !newv[w];
                    }
        ...
        // Bucket by canonical signature. Constant signatures (all-0/all-1) are dropped: they
        // are backbone-like (handled by the base sweep) and would form one giant useless
        // bucket. Key = min(sig, ¬sig); the stored literal sign records the orientation.
        let all_zero = [0u64; SIM_WORDS];
        let all_one = [u64::MAX; SIM_WORDS];
        let mut buckets: HashMap<[u64; SIM_WORDS], Vec<i32>> = HashMap::new();
        for v in 1..=num_vars {
            if self.assignment[v] != UNASSIGNED || self.eliminated[v] {
                continue;
            }
            let sig = values[v];
            if sig == all_zero || sig == all_one {
                continue;
            }
            let mut neg = sig;
            for w in 0..SIM_WORDS {
            ...
            let (key, pos) = if sig <= neg { (sig, true) } else { (neg, false) };
            buckets
                .entry(key)
                .or_default()
                .push(if pos { v as i32 } else { -(v as i32) });
        }

        // Emit candidate pairs: every bucket member against the bucket representative. Same
        // stored sign => candidate a<->b; opposite => candidate a<->¬b.
        ...
        for members in buckets.values() {
            ...
            let rep = members[0];
            let rv = rep.unsigned_abs();
            for &m in &members[1..] {
                if pairs.len() >= SIM_MAX_PAIRS {
                    return pairs;
                }
                let mv = m.unsigned_abs();
                if mv == rv {
                    continue;
                }
                let same = (rep > 0) == (m > 0);
                ...
                pairs.push((a, if same { b } else { -b }));
            }
        }
        pairs
    }

    /// Prove the simulation-discovered candidate pairs (bead SAT-playground-5b2.3.38). For
    /// each candidate `a ~ b`, builds a TWO-SEED environment (BFS from both members) so both
    /// fan-in cones are co-located in one kitten instance — the base per-variable sweep can
    /// never do this, because from either member the twin is many occurrence-graph hops
    /// away. The environment then goes through the exact same sound `prove_facts` +
    /// `proof_lemmas_outer` + ELS pipeline as `sweep_round`, so correctness is unchanged;
    /// only the *seeding* is new. Default off (SAT_SWEEP_SIM).
    fn sweep_simulation_round(&mut self, proof_log: &mut ProofLog) -> bool {
        use crate::sweep::{
            build_environment_multi, prove_facts, SWEEP_DEPTH, SWEEP_MAX_CLAUSES, SWEEP_MAX_VARS,
        };
        if self.current_level() != 0 || self.has_empty_clause || !self.solver_ok {
            return true;
        }
        ...
        let pairs = self.sweep_simulation_candidate_pairs();
        if pairs.is_empty() {
            return true;
        }
        self.stats.sweep_sim_pairs = self.stats.sweep_sim_pairs.saturating_add(pairs.len() as u64);

        // Snapshot live original clauses (len >= 2) — same owned-locals pattern as
        // sweep_round so the BFS closure does not borrow self while we later mutate it.
        let mut snap: Vec<Vec<i32>> = Vec::new();
        let mut occ: Vec<Vec<usize>> = vec![Vec::new(); nvars + 1];
        let ids = self.original_clause_ids.clone();
        for cid in ids {
        ...
        // One environment per candidate pair (not per variable), so afford a larger reach to
        // close both fan-in cones.
        let depth = env_usize("SAT_SWEEP_SIM_DEPTH", SWEEP_DEPTH.max(4));
        let max_vars = env_usize("SAT_SWEEP_SIM_MAX_VARS", SWEEP_MAX_VARS.max(512));
        let max_clauses = env_usize("SAT_SWEEP_SIM_MAX_CLAUSES", SWEEP_MAX_CLAUSES.max(4096));

        let mut all_backbones: Vec<i32> = Vec::new();
        let mut all_equivalences: Vec<(i32, i32)> = Vec::new();
        for (a, b_signed) in pairs {
            let av = a.unsigned_abs() as usize;
            let bv = b_signed.unsigned_abs() as usize;
            if av > nvars || bv > nvars {
                continue;
            }
            if self.assignment[av] != UNASSIGNED || self.eliminated[av] {
                continue;
            }
            if self.assignment[bv] != UNASSIGNED || self.eliminated[bv] {
                continue;
            }
            if occ[av].is_empty() || occ[bv].is_empty() {
                continue;
            }
            let mut env = build_environment_multi(
                &[a, b_signed.abs()],
                &clauses_of,
                depth,
                max_vars,
        ...
        // Apply the proven facts — identical to sweep_round.
        if !all_backbones.is_empty() {
            self.stats.sweep_backbones =
                self.stats.sweep_backbones.saturating_add(all_backbones.len() as u64);
            if !self.inprocess_add_root_units(&all_backbones, proof_log) {
    ...
    /// during-analyze approach in solver 12 would require a clause arena rewrite
...
mod tests {
    ...
    #[test]
    fn sweep_simulation_finds_equal_signature_candidate() {
        // Two AND gates 3=AND(1,2) and 4=AND(1,2). Simulation gives 3 and 4 the identical
        // K-bit signature => candidate (3,4). (5 keeps 3,4 from being propagated away.)
        let s = make_solver(
            5,
            vec![
                vec![-3, 1],
        ...
        let pairs = s.sweep_simulation_candidate_pairs();
        assert!(
            pairs.contains(&(3, 4)),
            "expected candidate (3,4) from equal signatures, got {pairs:?}"
        );
    }

    #[test]
    fn sweep_simulation_round_proves_and_applies_equivalence() {
        // End to end: the (3,4) candidate is proven by the two-seed environment + kitten and
        // the equivalence is applied. Every applied fact is UNSAT-proven, so this is sound.
        ...
        let mut proof = ProofLog::disabled();
        assert!(s.sweep_simulation_round(&mut proof));
        assert!(
            s.stats.sweep_equivalences >= 1,
            "expected 3<->4 proven, sweep_equivalences={}",
            s.stats.sweep_equivalences
        );
        assert!(s.stats.sweep_sim_pairs >= 1);
    }

    #[test]
    fn sweep_simulation_round_no_false_positive_on_independent_gates() {
        // 3=AND(1,2) and 4=AND(1,-2) compute different functions => different signatures =>
        // no candidate, nothing applied. Guards against applying a non-equivalence.
        let mut s = make_solver(
            5,
            vec![
                vec![-3, 1],
        ...
        assert_eq!(
            ...
            "independent gates must not be merged"
        );
    }
    ...
    fn test_temp_assumption_conflict_restores_propagate_head() {
        ...
        let start_temp_props = s.temporary_stats.propagations;
        ...
        assert_eq!(s.temporary_stats.propagations, start_temp_props + 1);
    ...
    #[test]
    fn test_normal_propagation_updates_normal_accounting_and_phase() {
        // Bead SAT-playground-5b2.2.61: the normal-search monomorphization
        // must preserve the ordinary enqueue/propagation accounting behavior
        // while avoiding the temporary-assumption policy checks in the hot path.
        let mut s = make_solver(2, vec![vec![2, 1]]);
        let start_props = s.stats.propagations;
        let start_temp_props = s.temporary_stats.propagations;
