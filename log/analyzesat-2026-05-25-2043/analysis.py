#!/usr/bin/env python3
"""analysis.py — Phase 2 + Phase 3 work × speed decomposition.

Reads per-config results.csv and stats.jsonl under log/<slug>/<config>/ and
produces:
  - matrix.csv  (config × instance with wall, conflicts, props, props/s, result)
  - decomp.csv  (work_ratio, speed_ratio, net, actual wall ratio vs A_baseline)
  - reference_gap.csv (per-instance kissat-latest vs A_baseline gap)
  - summary.md  (human-readable summary)

Designed for: log/analyzesat-2026-05-25-2043/.
"""
...
DEFAULT_SLUG_DIR = Path("/home/bojji/code/SAT-playground/log/analyzesat-2026-05-25-2043")
CONFIGS_ORDER = ["A_baseline", "B_metadata_only", "C_lbd_ema", "D_focused_stable",
                 "E_combined", "F_full_stack"]


def short_instance(name: str) -> str:
    # Strip directory and hash prefix and .cnf(.xz)? suffix.
...
def load_results(cfg_dir: Path):
    """Return dict[instance] = {"wall": float, "result": str, "verified": str, ...}."""
...
def load_stats(cfg_dir: Path):
    """Return dict[instance] = parsed JSON stats from stats.jsonl."""
...
def get_counter(stats: dict, *keys, default=0):
    """Look up a counter possibly nested in JSON_STATS."""
    if not stats:
        return default
    # Stats often have top-level 'counters' or fields directly.
    # Try keys directly, then under 'counters', 'stats'.
    for k in keys:
        if k in stats:
            return stats[k]
...
def main():
    ...
    ap.add_argument("--reference-csv", default="",
                    help="path to log/bench-kissat-latest-.../results.csv for Phase 3")
    ap.add_argument("--reference-stats", default="",
                    help="path to log/bench-kissat-latest-.../stats.jsonl (optional)")
    args = ap.parse_args()
    ...
    if not instances:
        print("ERROR: no results found under", slug_dir, file=sys.stderr)
        sys.exit(1)

    # Sample a stats record to discover counter keys.
    sample_stats = None
    for cfg in configs.values():
        for v in cfg["stats"].values():
            sample_stats = v
            break
    ...
    if sample_stats is None:
        print("WARN: no stats.jsonl content found — wall-only matrix", file=sys.stderr)
    ...
    with matrix_csv.open("w") as fh:
        ...
        for cfg in CONFIGS_ORDER:
            ...
            for inst in instances:
                ...
                rest = get_counter(s, "restarts", "restarts_total",
                                   "luby_restarts", "glucose_restarts",
                                   "reluctant_restarts")
                ...
                w.writerow([cfg, short_instance(inst), r.get("result", ""),
                            ...
                            conf, deci, props, f"{pps:.0f}", rest, ""])
    print(f"wrote {matrix_csv}")

    # Decomposition: ratio vs A_baseline per instance.
    ...
    with decomp_csv.open("w") as fh:
        ...
        w.writerow(["config", "instance", "result_A", "result_cfg",
                    ...
                    "props_A", "props_cfg",
                    "work_ratio_conf", "speed_ratio_pps", "net_pred",
    ...
    # Phase 3: reference comparison if provided.
    ...
    with summary_md.open("w") as fh:
        fh.write("# analyzesat summary\n\n")
        fh.write(f"Slug dir: {slug_dir}\n\n")
        fh.write("## PAR-2 per config (300s timeout, profiling suite)\n\n")
        ...
        fh.write("\nSee matrix.csv, decomp.csv, reference_gap.csv for details.\n")
